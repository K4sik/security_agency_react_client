...
import { DELETE_STATUS, GET_ERRORS, GET_STATUS, GET_STATUSES } from "./types";
...
export const getStatus = (status_id, history) => async dispatch => {
    try {
        const res = await axios.get(`http://localhost:8080/api/status/${status_id}`)
        dispatch({
            type: GET_STATUS,
            payload: res.data
        });
